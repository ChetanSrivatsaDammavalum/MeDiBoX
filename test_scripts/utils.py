...
# start function
def startup():
    """retrive customer database """    
    customer_data_base = cds.read_customerdata_file()
    return  cds.customerdata_to_record(customer_data_base)
...
def log_in_menu(customer_data):
    ...
    while True:
        if choice == 'y' or choice == 'Y':
            return login(customer_data)
        elif choice == 'n' or choice == 'N':
            return registration(customer_data)
        else:
...
def key_value_check(dict, dict_key, dict_value):
    if dict[dict_key]== dict_value:
        return True
    else: return False

def login(customer_data):
    """ login function - to check if email_id and password are correct """
    
    customer_check = False # flag - database exists
    value_check = False # flag - email_id exist/correct
    password_success = False # flag - password correct
    ...
    while True:
        ...
        if customer_check:
            ...
            while attempt < 3:
                ...
                password = input()
                value_check = key_value_check(customer_data,'password',password)
                if value_check:
                    password_success = True
                    break
                else:
                    attempt+=1
                    print(f'Wrong password, retry. attempts - {attempt}')
...
def main() -> None:
    ...
    #customer_data, customer_prescription = startup('email_id', email_id)
    
    #customer_data_base = startup()
    #print(customer_data_base)
    '''
    customer_data_base = cds.read_customerdata_file()
    customer_data = cds.read_customerdata('email_id','Abhi@gmail.com')
    print('customer database:', customer_data_base)
    print('customer data:', customer_data)
    '''
    response = cds.check_customerdata_from_file('email_id', 'Chetan@gmail.com','password','Chetan12345')
    print(response)
    ...
    #x, y = startup('email_id', 'Chetan@gmail.com')
    #print(x)
    #print(y)
